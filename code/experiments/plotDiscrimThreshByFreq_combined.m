...
for dd = 1:length(modDirections)
    ...
    for cc = 1:nContrasts % hi and low
        ...
        for ff = 1:numFreqs
            ...
            for ss = 1:length(sides) % hi and low
                ...
                psychObjArray{ss} = currentFile.psychObj;
            ...
            [~, psiParamsFit,psiParamsCI, psiParamsFitBoot] = psychObjArray{1}.reportCombinedParams(psychObjArray{2}, 'lb', lb, 'ub', ub, 'nBoots', nBoots, 'confInterval',confInterval);
            ...
            sensitivity(ff) = 1 / mu(ff);

            % Start by plotting one dot in each color for the legend
            if ff == 1
                dummy1 = plot(refFreqSetHz(1), sensitivity(1), plotSpec{1}, 'MarkerSize', 6,'MarkerFaceColor', markerColors{1});
                dummy2 = plot(refFreqSetHz(1), sensitivity(1), plotSpec{2}, 'MarkerSize', 6,'MarkerFaceColor', markerColors{2});
            end
        ...
        legend([dummy1 dummy2], {'Low Contrast' 'High Contrast'},'Location','northwest');
        ylim([0 1.2]);
