...
singleStimMat = zeros(4,stimVecLength);
...
for ii = 1:length(stimulusCoarse)
    if stimulusCoarse(ii)~=0
        ...
        singleStimMat(stimulusCoarse(ii),idx) = 1;
        if ii > 1
...
stimLabels = {};
for ii=1:nAcqs
    thisStimMat = zeros(6*nAcqs,stimVecLength);
    thisStimMat((ii-1)*6+1:ii*6,:) = singleStimMat;
