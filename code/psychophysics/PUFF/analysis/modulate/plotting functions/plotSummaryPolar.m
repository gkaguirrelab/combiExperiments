...
%    nexttile((cc-1)*(length(directionLabels)+1)+length(directionLabels)+1)
    for dd = 1:length(directionLabels)
        ...
        % We have an option here to flip the phase of the negative, S-cone
        % directed responses to allow all responses to be on the same side
        % of the polar plot
        if directionPhaseFlip{dd}
            phase = phase + pi;
        end
