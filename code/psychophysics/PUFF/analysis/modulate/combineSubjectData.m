...
phases = [0,pi];
contrasts =  {[0.2,0.2,0.2,0.2],[0.4,0.4,0.4,0.4]};
nTrials = 4;
...
% Get the fourier fits
fitResults = obtainFourierResults(results,...
        'directionLabels',directionLabels,...
        'phaseLabels',phaseLabels,...
        'contrastLabels',contrastLabels,...
        'nTrials',nTrials);
...
for dd = 1:length(directions)
    for cc = 1:length(contrasts)
        ...
        for ss = 1:length(subjects)
            % Get the bootstrap distributions
            bootAmplitude = fitResults.(directionLabels{dd}).(contrastLabels{cc}).bootAmplitude(ss);
            bootPhase = fitResults.(directionLabels{dd}).(contrastLabels{cc}).bootPhase(ss);
            plotPolarBivariateEllipse(bootAmplitude,bootPhase,'errorType','sd',...
