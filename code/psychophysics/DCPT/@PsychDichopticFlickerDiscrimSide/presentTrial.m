...
% Determine which interval will have the test flicker - always the testInterval.
refInterval = 1;
testInterval = 2;
...
if simulateMode
    ...
    %% Simulate
    sideChoice = obj.getSimulatedResponse(testParam,testInterval);
   ...
   stopTime = cputime() + obj.stimDurSecs;
    ...
    % Turn off the combis for the ISI
    obj.CombiLEDObjArr{2}.goDark;
    obj.CombiLEDObjArr{1}.goDark;
    ...
    if obj.useKeyboardFlag
        ...
        if ~isempty(buttonPress)
            switch buttonPress
                case {5 7}  % bumpers on left side of gamepad % was {5 6} in old version 
                    sideChoice = 1;
                case {6 8}  % bumpers on right side of gamepad % was {7 8} in old version 
                    sideChoice = 2;
            end
        end
    end
...
% test stimulus. Determine if the subject has selected the correct side
