...
file = '/Users/rubybouh/Aguirre-Brainard Lab Dropbox/Ruby Bouhassira/FLIC_data/combiLED/FLIC_0015/LightFlux_ND0x5_shifted/DCPT_SDT/FLIC_0015_LightFlux_DCPT_SDT_cont-0x3_refFreq-17.3205Hz_hi.mat';
load(file,'psychObj');
obj = psychObj; 
...
% FITTING MODEL TO DATA WITH MLE

% Getting data
dB_data = questData.trialData.stim; % vector of dB differences, one per trial
response_data = [questData.trialData.respondYes]; % 0 = "Same", 1 = "Different", same length as dB_data
...
for i = 1:length(dB_values)
    dB_value = dB_values(i);
    c = criterion(dB_value, m, b);
    compute_P_different(dB_value, sigma, c);
end
...
function nll = neg_log_likelihood(params, dB_data, response_data, sigma)
    m = params(1);
    b = params(2);
    nll = 0;
...
initial_params = [0.4, 1.6];  % e.g., b = 2 - m

% Run MLE to minimize negative log likelihood
best_params = fminsearch(@(p) neg_log_likelihood(p, dB_data, response_data, sigma), ...
                         initial_params);
