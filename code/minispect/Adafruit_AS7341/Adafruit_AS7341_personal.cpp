...
/**
 * @brief Sets the spectral and flicker detection AGC modes
 *
 * @param fifo_thresh, sp_agc_flag, fd_agc_flag, uint8_t
 *        values that represent the 
 * @return true: success false: failure
 */

bool Adafruit_AS7341::enable_AGC(uint8_t fifo_thresh, uint8_t sp_agc_flag, uint8_t fd_agc_flag) {
    Adafruit_BusIO_Register cfg8_reg = Adafruit_BusIO_Register(i2c_dev, AS7341_CFG8);

    if(fifo_thresh < 0 || fifo_thresh > 3) {
        return false; 
    }
    
    if(sp_agc_flag < 0 || sp_agc_flag > 1){
        return false; 
    }

    if(fd_agc_flag < 0 || sp_agc_flag > 1) {
        return false; 
    }

    fifo_thresh = fifo_thresh << 6;
    fd_agc_flag = fd_agc_flag << 3;
    sp_agc_flag = sp_agc_flag << 2; 
    
    uint8_t write_val = fifo_thresh + fd_agc_flag + sp_agc_flag; 
    

    return cfg8_reg.write(write_val);
}


/**
 * @brief Sets the AGC thresholds 
 *
 * @param high, low, flicker, uint8_t values represent high/low hysterisis settings, 
 *        flicker setting
 * @return true: success false: failure
 */

bool Adafruit_AS7341::setAGC(uint8_t high, uint8_t low, uint8_t flicker) {
  Adafruit_BusIO_Register cfg10_reg = Adafruit_BusIO_Register(i2c_dev, AS7341_CFG10);
  ...
  uint8_t write_val = high + low + flicker; 

  return cfg10_reg.write(write_val);
// bitfield [3] is empty
}
