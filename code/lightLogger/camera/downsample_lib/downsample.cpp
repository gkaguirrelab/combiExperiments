...
/*
Downsample a given 16-bit image by 2^factor along each dimension while 
being Bayer-aware. 
@Params: flattened_img - a pointer to the original image flattened
         rows, cols - integers representing the original size of the image 
         factor - integer representing the power of 2 to downsample by 
...
@Returns: None
*/
void downsample16(uint8_t* flattened_img_8, 
                  uint16_t rows, 
                  uint16_t cols,
                  uint8_t factor,
                  uint8_t* output_8) 
{       
    ...
    // Reinterpret the bytes of the 8-bit style image input and output to 16-bit style image 
    const uint16_t* flattened_img_16 = reinterpret_cast<uint16_t*>(flattened_img_8);
    uint16_t* output_16 = reinterpret_cast<uint16_t*>(output_8); 
    ...
    for(size_t r = 0; r < rows; r += chunk_size) {
        ...
        for(size_t c = 0; c < cols; c += chunk_size) {
            // Find the locations and values of each of the blue pixels in this chunk
            std::array<uint16_t, 4> b_pixels = { flattened_img_16[ pixel_to_index(r, c, cols) ], 
                                                 flattened_img_16[ pixel_to_index(r, c+2, cols) ],
                                                 flattened_img_16[ pixel_to_index(r+2, c, cols) ],
                                                 flattened_img_16[ pixel_to_index(r+2,c+2,cols) ] }; 
           
            // Find the locations and values of each of the green pixels on blue rows
            // in this chunk 
            std::array<uint16_t, 4> gb_pixels = { flattened_img_16[ pixel_to_index(r,c+1,cols) ], 
                                                  flattened_img_16[ pixel_to_index(r,c+3,cols) ],
                                                  flattened_img_16[ pixel_to_index(r+2,c+1,cols) ],
                                                  flattened_img_16[ pixel_to_index(r+2,c+3,cols) ] };
            
            // Find the locations and values of each of the red pixels in this chunk
            std::array<uint16_t, 4> r_pixels = { flattened_img_16[ pixel_to_index(r+1,c+1,cols) ], 
                                                 flattened_img_16[ pixel_to_index(r+1,c+3,cols) ],
                                                 flattened_img_16[ pixel_to_index(r+3,c+1,cols) ],
                                                 flattened_img_16[ pixel_to_index(r+3,c+3,cols) ] }; 

            // Find the locations and values of each of the green pixels on red rows 
            // in this chunk
            std::array<uint16_t, 4> gr_pixels = { flattened_img_16[ pixel_to_index(r+1,c,cols) ], 
                                                  flattened_img_16[ pixel_to_index(r+1,c+2,cols) ],
                                                  flattened_img_16[ pixel_to_index(r+3,c,cols) ],
                                                  flattened_img_16[ pixel_to_index(r+3,c+2,cols) ] }; 
            ...
            // Calculate the average of this color of pixels and cast back to unsigned 8 bit
            uint16_t b_average = static_cast<uint16_t>(b_sum >> 2);
            uint16_t gb_average = static_cast<uint16_t>(gb_sum >> 2);
            uint16_t r_average = static_cast<uint16_t>(r_sum >> 2);
            uint16_t gr_average = static_cast<uint16_t>(gr_sum >> 2);

            // Set the downsampled image to have their values
            output_16[ pixel_to_index(downsampled_r, downsampled_c, new_cols) ] = b_average; 
            output_16[ pixel_to_index(downsampled_r, downsampled_c+1, new_cols) ] = gb_average; 
            output_16[ pixel_to_index(downsampled_r+1, downsampled_c+1, new_cols) ] = r_average; 
            output_16[ pixel_to_index(downsampled_r+1, downsampled_c, new_cols) ] = gr_average; 
